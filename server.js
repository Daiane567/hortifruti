...
app.post('/produtos', (req, res, next) => {
    if (req.body.vendedor === undefined || req.body.vendedor.id === undefined) {
        next(res.json({ mensagem: "Parametro id do vendedor não informado" }).status(400));
    }
    ...
    if (vendedor === undefined) {
        next(res.json({ mensagem: "vendedor não encontrado" }).status(404));
    ...
    next(res.json({ mensagem: "produto cadastrado com sucesso", resultado, "id": id }).status(201));
